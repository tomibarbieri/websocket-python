...
class WebSocketHandler(tornado.websocket.WebSocketHandler):
	...
	def test(self):
		...
		try:
			...
			try:
				info = {
					"potencia"  : float(random.randrange(40,80)),#float(values[2]),
					"tension"   : float(random.randrange(40,80)),#float(values[4]),
					"corriente" : float(random.randrange(40,80)),#float(values[6]),
					"estado"    : float(1),#int(values[8]),
					"timestamp" : time.time()
				}
			...
			tornado.ioloop.IOLoop.instance().add_timeout(timedelta(seconds=0.5),self.test)
	...
	def on_message(self, message):
		...
		try:
		    ...
		    if "relay1" in message:
		        if (message['relay1'] == 1):
					print("queres prender guacho")
					#ser.write("1\r".encode())
		        elif (message['relay1'] == 0):
					print("queres apagar guacho")
		    ...
		    if "max1" in message:
		        if (message['max1'] >= 0 and message['max1'] <= 30):
					print("te pasaste del maximo amigo")
